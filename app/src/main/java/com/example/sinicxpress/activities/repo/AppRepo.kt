package com.example.sinicxpress.activities.repo

import android.net.Uri
import android.util.Log
import com.example.sinicxpress.activities.utility.State
import com.example.sinicxpress.activities.SavedPlaceModel
...
import com.example.sinicxpress.activities.network.RetrofitClient
...
class AppRepo {
    ...
    fun forgetPassword(email: String) = flow<State<Any>> {
        emit(State.loading(true))
        val auth = Firebase.auth
    ...
    fun getPlaces(url: String) = flow<State<Any>> {
        emit(State.loading(true))
    ...
    fun getDirection(url: String) = flow<State<Any>> {
        emit(State.loading(true))
    ...
    fun updateName(name: String) = flow<State<Any>> {
        emit(State.loading(true))
    ...
    fun updateImage(image: Uri) = flow<State<Any>> {
        emit(State.loading(true))
    ...
    fun confirmEmail(authCredential: AuthCredential) = flow<State<Any>> {
        emit(State.loading(true))
    ...
    fun updateEmail(email: String) = flow<State<Any>> {
        emit(State.loading(true))
    ...
    fun updatePassword(password: String) = flow<State<Any>> {
        emit(State.loading(true))
